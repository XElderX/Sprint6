...
if(isset($_POST['back'])){
    ...
    ? print "<p class='eventNote'> Nowhere to go back  </p>"
    ...
    if(str_contains($_newFile , ".")){
        ...
        ? ((is_dir($path))
            ? fopen($path . $_newFile, "w") and header("Refresh:0") and exit
            : null)
        : print "<p class='eventNote' style='color:red; font-size:24px; font-weight:bold'> File with same file name is already exist!!!</p>";
       }
    else{
    $path = modifiedPath ();
    (!file_exists($path . $_newFile ))
    ? ((is_dir($path)) 
        ? mkdir($path . $_newFile, 0777,TRUE) and print "<p class='eventNote'>directory with the name $_newFile has been created </p>" and header("Refresh:3")
        : null)   
    :  print "<p class='eventNote' style='color:red; font-size:32px; font-weight:bold'>Directory with same directory name is already exist!!!</p>" and header("Refresh:3");  
       }
   }
if(isset($_POST['delete'])){
    ...
    ? ((is_file($path . $fileToDelete)) 
        ? ((!unlink($path . $fileToDelete)) 
            ? print (" <p class='eventNote'> style='color:red; font-size:32px; font-weight:bold'> $fileToDelete cannot be deleted due to an error </p>") and header("Refresh:3")
            : print ("<p class='eventNote'> $fileToDelete has been deleted </p>") and header("Refresh:2"))  
        :  ((isDirEmpty(($path . $fileToDelete)))
            ? rmdir($path . $fileToDelete) and print ("<p class='eventNote'>  $fileToDelete has been deleted </p>") and header("Refresh:3")
            : print ("<p class='eventNote' style='color:red; font-size:32px; font-weight:bold'> $fileToDelete cannot be deleted due to an error: check if the directory is empty?</p>") and header("Refresh:5"))) 
    : print (" <p class='eventNote' style='color:red; font-size:32px; font-weight:bold'> Are you crazy??? <span style = 'background-color:yellow'> $fileToDelete </span> is forbidden to delete!!! </p>"); 
}
}   ?>
<!DOCTYPE html>
<html lang="en">
...
<div class = "container2">
    ...
    <div class='dirLine' > <span><b> Currently in </b>" . substr_replace(modifiedPath (),'',-1) . " " . "<b>directory</b></span></div>";
        print  ("
    <section><div class='tableContainer'>
    <table><tr><th>Type</th><th>Name</th><th>Actions</th></tr>");
            showFiles();
        print "
    </table></div>";
            generateForm();
